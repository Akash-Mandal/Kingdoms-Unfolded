...
var base_url: String = "http://127.0.0.1:11434"
var _tokens_used := 0
...
func _ready() -> void:
	...
	if vault != null and vault.has_method("load_config"):
		var cfg: Dictionary = vault.call("load_config", "ollama") as Dictionary
		if cfg.has("model"):
			model = str(cfg["model"])
...
func _endpoint() -> String:
	return "%s/api/chat" % base_url.rstrip("/")
func _build_body(prompt: Dictionary) -> String:
	...
	var system_text := "You are the chronicler of %s. Turn %s season %s. Respond ONLY with valid JSON {text, choices[{id,label,hint,effects}], hooks[]}." % [
		str(ctx.get("kingdom_name", "Eterna")), str(ctx.get("turn", 0)), str(ctx.get("season", "spring"))
	]
	var user_text: String = str(prompt.get("task", event.get("text", "Narrate this event.")))
	var schema := '{"type":"object","required":["text","choices","hooks"],"properties":{"text":{"type":"string"},"choices":{"type":"array","items":{"type":"object","required":["id","label"],"properties":{"id":{"type":"string"},"label":{"type":"string"},"hint":{"type":"string"}}}},"hooks":{"type":"array","items":{"type":"string"}}}}'
	return JSON.stringify({
		...
		"messages": [
			{"role": "system", "content": system_text + " Schema: " + schema},
...
func _on_http_done(result: int, code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if content_text == "":
		content_text = str(dict.get("response", txt))
